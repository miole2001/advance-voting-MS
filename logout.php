...
if (isset($_COOKIE['user_id'])) {
    ...
    $verify_user = $connAccounts->prepare("SELECT email, user_type FROM `user_accounts` WHERE id = ?");
    ...
    if ($user) {
        ...
        if ($user_type === 'admin') {
            ...
            $log_stmt = $connLogs->prepare("INSERT INTO admin_logs (email, activity_type, user_type) VALUES (?, 'Logout', ?)");
        } else if ($user_type === 'user') {
            // Prepare statement for user logs
            $log_stmt = $connLogs->prepare("INSERT INTO user_logs (email, activity_type, user_type) VALUES (?, 'Logout', ?)");
        } else {
